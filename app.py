...
dist_names = ['t','triang','weibull_min','weibull_max','rayleigh','pareto', 'norm',
              'logistic','laplace','gompertz','gamma','f',
              'genextreme','expon','chi2','beta','cauchy']
                ...
                # html.Div([
                    ...
                    [
                        ...
                        html.Div([
                            ...
                            html.Div(
                            [
                                html.Button('CLEAN UP DATA (IQR METHOD)', id='IQR_btn', n_clicks=0, style={'box-sizing':'border-box','margin-top': '20px', 'display': 'inline-block','background-color': theme['button_background'],
                                    'color': theme['dark_color']}),
                            ],),
                            html.Div(
                            [
                                html.Button('DECREASE NUMBER OF SAMPLES', id='DECREASE_btn', n_clicks=0, style={'box-sizing':'border-box','margin-top': '20px', 'display': 'inline-block','background-color': theme['button_background'],
                                    'color': theme['dark_color']}),
                            ], ),
                            html.Div(
                            [
                                html.Button('SMOOTH TIME SERIES', id='SMOOTH_btn', n_clicks=0, style={'box-sizing':'border-box','margin-top': '20px', 'display': 'inline-block','background-color': theme['button_background'],
                                    'color': theme['dark_color']}),
                            ],),
                            html.Div(
                            [
                                html.Button('UNDO ABOVE OPERATIONS', id='UNDO_btn', n_clicks=0, style={'box-sizing':'border-box','margin-top': '20px', 'display': 'inline-block','background-color': theme['button_background'],
                                    'color': theme['dark_color']}),
                            ],),
                        ],
                        style={
                ...
                html.Div([
                    ...
                    dte.DataTable(
                        ...
                        columns=[{"name": "standard deviation", "id": "1"},
                                 {"name": "variance", "id": "2"},
                                 {"name": "coefficient of variation", "id": "3"}],
                        style_header={'backgroundColor': 'black'},
                        style_cell={
                            'backgroundColor': theme['graph_background'],
                            'color': '#E0E0E0'
...
def update_value(n_clicks, content, filename, _value, time_series, v_years, v_months, v_days, v_days_of_week, o_years, o_months, o_days, o_days_of_week):
    global df
    ctx = dash.callback_context

    if ctx.triggered[0]['prop_id'] == 'statistics_btn.n_clicks' and time_series is not None:
        return get_statistical_properties(v_years, v_months, v_days, v_days_of_week, o_years, o_months, o_days, o_days_of_week, time_series)
...
def get_statistical_properties(v_years, v_months, v_days, v_days_of_week, o_years, o_months, o_days, o_days_of_week, time_series):
    global df
    filtered_df = None
    rows1 = []
    rows2_1 = []
...
def get_median_absolute_deviation(data):
    try:
        if len(data) == 0:
            return "impossible to measure"
        return scipy.stats.median_absolute_deviation(data)
    except:
        return "impossible to measure"
...
def get_quartile_deviation(data):
    try:
        if len(data) == 0:
            return "impossible to measure"
        return (np.percentile(data, 75) - np.percentile(data, 25)) / 2
    except:
        return "impossible to measure"
...
def update_download_link(n_clicks0, n_clicks1, n_clicks_2, properties_indexes, columns, property_options, years, months, days, days_of_week, o_years, o_months, o_days, o_days_of_week, csv_string):
    ...
    try:
        ...
        if ctx.triggered[0]['prop_id'] == 'report_btn.n_clicks':
            ...
            for col in columns:
                properties_column = get_statistical_properties(years, months, days, days_of_week, o_years, o_months, o_days, o_days_of_week, col)
    ...
    try:
        ...
        data.extend(
            (properties_column[2][0]["1"], properties_column[2][0]["2"], properties_column[2][0]["3"], properties_column[2][0]["4"],
             properties_column[2][0]["5"], properties_column[2][0]["6"]))
        data.extend(
        ...
        data.extend(
            (properties_column[5][0]["1"], properties_column[5][0]["2"], properties_column[5][0]["3"]))
        data.extend(
...
@app.callback(
    ...
    [Input('correct_data_btn', 'n_clicks'),
     ...
     [State('output-graph', 'figure'),
      ...
      State('dropdown_columns', 'value')],)
def update_value(n_click_1, n_click_2, value, n_click_3, n_click_4, n_click_5, n_click_6, n_click_7, n_click_8, n_click_9, n_cick_10, figure, rows, color, dropdown_color, dropdown_value):
    global df
    global tmp_df
    ...
    if ctx.triggered[0]['prop_id'] == 'reset_btn.n_clicks':
        ...
        if df_reset is not None:
            ...
            is_normalized = False
            was_changed = False
            return get_graph(dropdown_value,df)
        # except:
        ...
        if df is not None:
            ...
            if len(current_values) > 2:
                for k in range(len(current_values)):
                    if k % 2 == 0:
                        count += 1
                new_df.drop(new_df.tail(len(current_values) - count).index, inplace=True)
...
def best_distrubution(time_series):
    global dist_names
    ...
    number_of_bins = 100
    bin_cutoffs = np.linspace(np.percentile(data, 0), np.percentile(data, 99), number_of_bins)

    # Create the plo

    # Get the top 1 distributions from the previous phase
    number_distribution = 1
    chosen_dist_names = results['Distribution'].iloc[0:number_distribution]
    ...
    for dist_name in chosen_dist_names:
    ...
    dist_parameters['Distribution'] = (
        results['Distribution'].iloc[0:number_distribution])
    ...
    for i in range(len(shapes)):
        tmp = str(shapes[i]) + " = " + str(parameters[0][i])
        returned_value += tmp
        if i < (len(shapes)-1):
            returned_value += ", "
...
def update_value(n_clicks_1,n_clicks_2,value,time_series):
    ...
    if ctx.triggered[0]['prop_id'] == 'distribution_btn.n_clicks':
        try:
            x = best_distrubution(time_series)
        except:
